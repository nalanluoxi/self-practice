...
HashMap 底层通过 hash(key) & (table.length - 1) 计算数组下标，要求数组长度 cap 为 2 的 n 次方，核心三点：
1. 高效取模，替代 % 运算
   正常取模公式：hash % cap
   当 cap = 2^n 时：
   hash & (cap - 1) 等价于 hash % cap
   位运算速度远高于取模运算，提升寻址性能。
   例：cap=16 (10000)，cap-1=15 (01111)，与运算直接截取 hash 低 4 位，实现取模。
2. 减少哈希冲突，散列分布均匀
   若容量不是 2 的幂，cap-1 二进制会存在 0 位，与运算后 hash 对应位永远为 0，大量 key 会映射到同一下标，冲突激增。
   例：cap=15 (01111)，cap-1=14 (01110)，二进制末位固定为 0，所有奇数 hash 都会丢失，只能落到偶数下标，数组一半空间浪费。
3. 扩容时快速重哈希（rehash）
   扩容规则：容量翻倍（仍为 2 次幂）。
   旧下标 i 只有两种结果：
   新下标 = i
   新下标 = i + 旧容量
   不需要重新计算 hash，只判断 hash 新增高位是 0/1，直接转移链表 / 红黑树节点，大幅提升扩容效率。
   补充：如果传入非 2 次幂初始化容量怎么办？
   HashMap 构造器会通过 tableSizeFor() 向上取最近的 2 的幂。比如传入 13，自动修正为 16。

## G1→CMS 为啥改

CMS 是并发低停顿收集器，但存在致命缺陷，JDK9 默认 G1，JDK14 废弃 CMS，核心痛点如下：
1. CMS 存在内存碎片，Full GC 卡顿严重
   CMS 只清理老年代，不做压缩，长期运行老年代产生大量碎片。
   当大对象分配时没有连续内存，触发 STW Full GC，单线程整理堆，停顿时间不可控，大型应用卡顿爆炸。
   G1：回收后自动做区域压缩，几乎无内存碎片，大对象分配稳定。
2. CMS 并发阶段浮动垃圾 & 并发失败（Concurrent Mode Failure）
   CMS 并发标记阶段，用户线程持续产生新垃圾，无法本轮回收（浮动垃圾）；
   若老年代内存占用过快，预留空间不足，触发并发失败，直接退化为 Serial Old 单线程 Full GC，长时间 STW。
   G1 采用分区域增量回收，每次只回收部分 Region，预留回收空间更充足，并发失败概率大幅降低。
3. CMS 堆内存必须连续，堆扩容不灵活
   CMS 把堆分为连续年轻代、连续老年代，堆越大，碎片问题越严重；超大堆场景缺陷放大。
   G1 将整个堆切分为多个等大 Region（Eden/Survivor/Old/Humongous 大对象区），内存不要求连续，灵活管理大对象，专门 Humongous 区存储超大对象，避免碎片。
4. CMS 吞吐量差，CPU 占用高
   CMS 全程多轮并发标记、并发清扫，占用大量 CPU；
   G1 做了吞吐量与停顿的平衡，可通过 -XX:MaxGCPauseMillis 指定预期停顿时间，可控低延迟。
5. 总结 G1 核心优势
   可预测的停顿时间模型；
   分区回收，解决内存碎片；
   内置大对象专用 Region；
   降低 Full GC 触发概率，Full GC 为多线程整理；
   堆内存无需连续，适配超大堆服务。
   拓展：更高版本 ZGC/Shenandoah 是更低停顿，但 G1 是折中通用方案，替代 CMS。

## 介绍，mysql的MVCC，如何实现多版本并发控制的

1. MVCC 是什么
   Multi-Version Concurrency Control，多版本并发控制。
   核心作用：读不加锁，读写不冲突，实现 RC/RR 隔离级别，提升并发性能。
   仅 InnoDB 支持，基于undo log 版本链 + read view 读视图实现。
2. 前置关键存储结构
   （1）隐藏字段（每条行记录自带）
   DB_TRX_ID：6 字节，最后修改这条数据的事务 ID；
   DB_ROLL_PTR：7 字节，回滚指针，指向 undo log 里上一个旧版本记录；
   DB_ROW_ID：6 字节，无主键时自动生成隐藏主键。
   （2）undo log 回滚日志
   事务修改数据前，先把旧版本拷贝到 undo log；
   通过 roll_ptr 串联所有历史版本，形成版本链表；
   事务回滚时，用 undo log 恢复数据；旧版本还用于 MVCC 快照读。
   （3）Read View 读视图（核心快照机制）
   执行快照读（普通 select 不加锁）时生成，保存 4 个核心数据：
   m_ids：当前活跃未提交事务 ID 集合；
   min_trx_id：m_ids 中最小事务 ID；
   max_trx_id：下一个将要分配的事务 ID；
   creator_trx_id：当前执行查询事务的 ID。
3. 可见性判断规则（RR/RC 隔离区别核心）
   拿记录的 DB_TRX_ID 和 Read View 对比，判断当前版本是否可见：
   trx_id < min_trx_id：事务已提交，版本可见；
   trx_id ≥ max_trx_id：事务查询后才开启，版本不可见；
   min_trx_id ≤ trx_id < max_trx_id：
   若 trx_id 在 m_ids（活跃未提交）：不可见；
   不在 m_ids（已提交）：可见；
   若 trx_id == 当前事务 creator_trx_id：自己修改的数据，可见。
   不可见则顺着 roll_ptr 找 undo log 上一个历史版本，循环判断直到找到可见版本。
4. RC 和 RR 隔离级别 MVCC 区别
   读已提交 RC
   每次 select 快照读，重新生成 Read View；
   能读到其他事务已提交的最新数据，存在不可重复读。
   可重复读 RR（MySQL 默认）
   同一个事务内，第一次 select 时生成 Read View，全程复用；
   整个事务看到同一快照，解决不可重复读；
   通过 Next-Key Lock 间隙锁解决幻读。
5. 当前读 vs 快照读（MVCC 适用场景）
   快照读（不加锁，走 MVCC 多版本）
   普通 select * from table;，读历史快照，读写不阻塞。
   当前读（加行锁，读取最新数据）
   select ... for update /lock in share mode /update/delete；
   读取最新已提交版本，加锁，读写互斥。
6. MVCC 整体执行流程
   事务执行普通 select，生成 Read View；
   读取聚簇索引行数据，拿到 DB_TRX_ID；
   根据 Read View 规则判断当前版本是否可见；
   不可见则通过 roll_ptr 遍历 undo log 版本链；
   找到第一条可见版本，返回给客户端；
   事务提交后，undo log 旧版本等待 purge 线程清理。
7. MVCC 优缺点
   优点：
   读无锁，读写并发不阻塞，高并发性能强；
   读操作不产生死锁。
   缺点：
   需要存储大量 undo log，占用磁盘；
   快照读需要遍历版本链，查询性能轻微损耗；
   purge 线程定期清理过期 undo，增加后台开销。
